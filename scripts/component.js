...
const dir = path.resolve(
  __dirname,
  [COMPONENTS_FOLDER, targetfolder, name].join('')
);
...
function write() {
  mkdirp(dir, err => {
    ...
    const files = [
      template(
        path.resolve(__dirname, 'templates/' + type + '/Component.js'),
        path.resolve(dir, `${name}.js`)
      ),
      template(
        path.resolve(__dirname, 'templates/' + type + '/Component.global.scss'),
        path.resolve(dir, `${name}.global.scss`)
      )
      // template(
